function [graph] = dbrent(a0,b0,c0,f,df,tol)
...
while ((count < maxits) && (abs(x(1)-xm)>tol2-.5*(b-a))) && (exit == 0)
    if abs(e)>tol1
        ...
        [u,a,b,v,w,x,exit,count] = block2(d,tol1,x,a,b,w,v,f,df,exit,count);
    else
        [e,d] = block1(x,a,b);
        [u,a,b,v,w,x,exit,count] = block2(d,tol1,x,a,b,w,v,f,df,exit,count);
    end
%     hold on
    graph = [graph,[count;x(1);x(2)]];
%     semilogy(count,abs(x(2)-1),[symbols(symbolcount),colors(colorcount)]);
    xm = .5*(a+b);
    tol1=tol*abs(x(1))+zerobuffer;
    tol2=2.*tol1;
%     [a,b]
...
function [u,a,b,v,w,x,exit,count] = block2(d,tol1,x,a,b,w,v,f,df,exit,count)
u=zeros(1,3);
if abs(d) >= tol1
    u(1)=x(1)+d;
    u(2)=feval(f,u(1));
