function [mini,fmin,count] = brentdir(a0,b0,c0,fnct,dirr,spot,tol)
...
while (count <= maxits)&&(abs(x-xm) > tol2-.5*(b-a))
    ...
    if abs(disttrav)>tol1
        ...
        if (abs(p)>=abs(.5*q*disttravtemp))||(p<=q*(a-x))||(p>=q*(b-x))
            ...
            [u,fu,a,b,v,fv,w,fw,x,fx,count]=block2(d,tol1,x,fx,w,fw,a,b,v,fv,fnct,count,dirr,spot);
        else
            d = p/q;
            u = x+d;
...
function [u,fu,a,b,v,fv,w,fw,x,fx,count]=block2(d,tol1,x,fx,w,fw,a,b,v,fv,fnct,count,dirr,spot)
if abs(d)>=tol1
    u = x+d;
else
    u = x+tol1*sign(d);
end
fu=feval(fnct,u*dirr+spot);
count = count + 1;
if fu <= fx
    if u >= x
        a = x;
