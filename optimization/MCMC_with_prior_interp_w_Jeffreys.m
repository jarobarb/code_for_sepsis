function MCMC_with_prior_interp_w_Jeffreys(varargin)
...
% works.
  ...
  save_file = 1;
  meas_window = [0,1];
  param_window_frac = [0.01,100];
  my_meas_sigm_mult = 1;
  my_para_sigm_mult = 1;
  interp_in_log_space = 1;
  interpolate = 10;
  ...
  SGV_consts.bic.val = bic;
  SGV_consts.mic.val = mic;
  SGV_consts.nic.val = nic;
  SGV_consts.cpic.val = cpic;
  SGV_consts.caic.val = caic;
  
  param_vals = SGV_consts;
  
  %  Initial guess/baseline parameter values used for other calculations  
  for fpc = 1:length(SGV_consts.free)
    free_params{fpc} = SGV_consts.free(fpc).name;
    pnorm(fpc) = SGV_consts.(free_params{fpc}).val;
  end
  [free_params,indices] = sort(free_params);
  pnorm = pnorm(indices);
  
  %  Range of the parameters allowed 
  for fpc = 1:length(pnorm)
    prange(fpc,:) = pnorm(fpc)*param_window_frac;
  end
  for vac = 1:2:length(varargin)
    temp_str = varargin{vac};
    if strncmpi(temp_str,'range_',5) == 1
      temp_str = temp_str(7:end);
      if strncmpi(temp_str,'frac_',4) == 1
        temp_param_name = temp_str(6:end);
        fp_ind = strmatch(temp_param_name,free_params);
        if isempty(fp_ind)
          warning(['The parameter ',temp_param_name,' is not free!']);
        else
          prange(fp_ind,:) = pnorm(fp_ind)*varargin{vac+1};
        end
      elseif strncmpi(temp_str,'abs_',3) == 1
        temp_param_name = temp_str(5:end);
        fp_ind = strmatch(temp_param_name,free_params);
        if isempty(fp_ind)
          warning(['The parameter ',temp_param_name,' is not free!']);
        else
          prange(fp_ind,:) = varargin{vac+1};
        end
      end
    end
  end
  ...
  run_name = ['MH_MCMC_N_',num2str(N),'_rd_',num2str(RD),'_ipp_',...
    num2str(init_param_pert),'sb_',num2str(sim_bump),...
    'mn',num2str(mysigm),'.mat'];
  fprintf('%s\n',run_name);
  
  %  Initialize experimental data and interpolation structure (if there is
  %    one)
  [Enorm,interp_struc,exp_data] = ...
    max_likelihood_prog_w_interp_new(pnorm,prange,free_params,[],[],...
    'meas_stddev',mysigm,'ets',linspace(0,100,11),'real_data',RD,...
    'interpolate',interpolate,'exp_plot',0,...
    'interp_in_log_space',interp_in_log_space,'dir_name',dir_name);
          
  if ~isdir('my_runs')
    [SUCCESS,MESSAGE,MESSAGEID] = mkdir('my_runs');
  end
  save(['my_runs',ms,'current_info.mat']);
  
  current_run_info = load(['my_runs',ms,'current_info.mat']);
  run_file_list = dir(['my_runs',ms,'run_info*']);
  run_info_index = length(run_file_list)+1;
  
  for rflc = 1:length(run_file_list)
    temp_run_info = load(['my_runs',ms,'run_info_',num2str(rflc),'.mat']);
    temp_run_info.mygrid.time_info = rmfield(temp_run_info.mygrid.time_info,'t00');
    temp_run_info = rmfield(temp_run_info,'rerun');
    try
      current_run_info.mygrid.time_info = rmfield(current_run_info.mygrid.time_info,'t00'); 
      current_run_info = rmfield(current_run_info,'rerun');
    end
    if isequal(current_run_info,temp_run_info);
      run_info_index = rflc;
      if rerun == 0
        final_fig_num_temp = final_fig_num;
        load(['my_runs',ms,'run_data_',num2str(rflc),'.mat']);
        run_file_list = dir(['my_runs',ms,'run_info*']);
  ...
  if (run_info_index == length(run_file_list)+1) || (rerun == 1)
%     pnorm = pnorm';
    p = repmat(pnorm,Nc,1);
...
%     end
    ...
    mysigj = mysigp;
    %  This is the standard deviation needed in the gaussian that appears
    %  in the exponent of the transition probability function in order for
    %  the transition probability to have a standard deviation of mysigj
    mysigj_exp = sqrt(log((1/2)*(p(1,:)+sqrt(p(1,:).^2+4*mysigj.^2))./p(1,:)));
    ...
    for kk = 1:Nc
      ...
      if SGV == 12
        ...
        if kk == 1
          [Ea(kk,1),interp_struc,exp_data] = ...
            max_likelihood_prog_w_interp_new(p(kk,:),prange,...
            free_params,[],[],'meas_stddev',mysigm,...
            'ets',linspace(0,100,11),'real_data',RD,...
            'interpolate',interpolate,'exp_plot',0,...
            'interp_in_log_space',interp_in_log_space,...
            'dir_name',dir_name);
          
%           (1,...
%             {'mlp_arg_pairs',{'stddev',mysigm},...
%             'sim_data_params',[p(kk,:)],'sim_ts',linspace(0,100,11),...
%             'min_frac',param_window_frac(1),...
%             'max_frac',param_window_frac(2)},...
%             p(kk,:), free_params);
          p0 = p;
        else
          Ea(kk,1) = Ea(1,1);
    ...
    while j < N
      ...
      for ll = 1:Nrs  %run each chain Np times before swapping
        ...
        pn_parfor = p.*exp(epsilon'*normrnd(0,mysigj_exp));    %generate new set of parameters
        ...
        for kk = 1:Nc                    
          ...
          if kk == 1
            Eb_parfor = ...
              max_likelihood_prog_w_interp_new(pn_parfor,prange,...
              free_params,interp_struc,exp_data,...
              'meas_stddev',mysigm,'ets',linspace(0,100,11),...
              'real_data',RD,'interpolate',interpolate,...
              'interp_in_log_space',interp_in_log_space,...
              'dir_name',dir_name);
%             Eb_parfor = max_likelihood_prog_w_interp(0,...
%               {'mlp_arg_pairs',{'stddev',mysigm},...
%               'min_frac',param_window_frac(1),...
%               'max_frac',param_window_frac(2)},...
%               pn_parfor, free_params);
          end
          Eb = Eb_parfor(kk);
...
%           fprintf('%g\n',h);
        ...
        if SGV == 11
          ...
          for ac = 1:length(accepted)
            fprintf('%g/',accepted(ac));
            fprintf('%g ',accepted(ac)+rejected(ac));
          end
          fprintf('accratios \n');
        end
      end
      ...
      if j-my_adjust_jumps_count*adjust_jumps_num > adjust_jumps_num
        ...
        tempsig = std(double(pvec(:,:,1:j)),0,3);
        tempmean_prim_chain = mean(double(pvec(1,:,1:j)),3);
        normsig = bsxfun(@ldivide,pnorm,tempsig)';
        normsig_prim_chain = normsig(:,1);
        [min_norm_sig,min_index] = min(normsig_prim_chain);
        std_dev_str = num2str(normsig);
        fprintf('Normalized standard deviation in current chain:\n');
        for pc = 1:numel(mysigj_exp)
        ...
        fprintf('\n');
        fprintf('%g/%g steps\n',j,N);
        k = curr_accratio(1)/desired_accratio;
        
        baseline_eps = sqrt(log(1/2+(1/2)*sqrt(1+...
          4*k^2*(exp(baseline_eps.^2*mysigj_exp(min_index).^2)).^2-...
          4*k^2*exp(baseline_eps.^2*mysigj_exp(min_index).^2))))./...
          mysigj_exp(min_index);
        if min(normsig_prim_chain) == 0
%           mysigj = mysigj/2;
        else
          mysigj = (normsig_prim_chain'./min_norm_sig).*...
            (mysigj(min_index)/pnorm(min_index)).*pnorm;
        end
        mysigj_exp = sqrt(log((1/2)*...
          (tempmean_prim_chain+sqrt(tempmean_prim_chain.^2+...
          4*mysigj.^2))./tempmean_prim_chain));
...
%       end
    ...
    %save data in a file
    copyfile(['my_runs',ms,'current_info.mat'],...
        ['my_runs',ms,'run_info_',num2str(run_info_index),...
        '.mat']);
    save(['my_runs',ms,'run_data_',num2str(run_info_index)]);
  end
  ...
  if size(pvec,2) > 1
    ...
    temp1 = pvec(:,1,:);
    temp2 = pvec(:,2,:);
    temp3 = exp(-(Evec(:)-min(Evec(:))));
    scatter3(temp1(:),temp2(:),temp3(:),'*');
%     plot(temp1(:)./pnorm(1),temp2(:)./pnorm(2),'.','MarkerSize',1);
    if log_switch == 1, set(gca,'XScale','log'); set(gca,'YScale','log'); end
    xlabel([free_params{1},' estimate/',free_params{1},' exact']);
    ylabel([free_params{2},' estimate/',free_params{2},' exact']);
  end
  
  [~,max_lik_est_ind] = max(temp_likelihood_MCMC(1,:));
  
  max_lik_ps = pvec(1,:,max_lik_est_ind);
  
  bad_ps = [3.316,0.0174];
  
  [~,good_ps_ind] = max((temp1(:)>3).*temp3(:));
  good_ps = [temp1(good_ps_ind),temp2(good_ps_ind)];
  
%   [bad_comp_xray_ts] = lung_model(bad_ps,free_params,exp_data.ts,...
%     exp_data.g,'plot_soln',1);
  [mle_comp_xray_ts] = lung_model(max_lik_ps,free_params,exp_data.ts,...
    exp_data.g,'plot_soln',1);
%   [good_comp_xray_ts] = lung_model(good_ps,free_params,exp_data.ts,...
%     exp_data.g,'plot_soln',1);
  gcf = figure(1073);
  set(gcf,'units','inches');
  set(gcf,'Position',[0.5,0.5,12,8]);
  
%   zmin = min([bad_comp_xray_ts(:);mle_comp_xray_ts(:);good_comp_xray_ts(:)]);
%   zmax = max([bad_comp_xray_ts(:);mle_comp_xray_ts(:);good_comp_xray_ts(:)]);
  zmin = min([mle_comp_xray_ts(:);exp_data.exp_xray_ts(:)]);
  zmax = max([mle_comp_xray_ts(:);exp_data.exp_xray_ts(:)]);
  
%   for tc = 1:length(exp_data.ts)
%     subplot(3,length(exp_data.ts),tc);
%     pcolor(padarray(mle_comp_xray_ts(:,:,tc),[1 1],'post'));
%     title('Exp');
%     caxis([zmin,zmax]);
%     
%     subplot(3,length(exp_data.ts),tc+length(exp_data.ts));
%     pcolor(padarray(bad_comp_xray_ts(:,:,tc),[1 1],'post'));
%     title('MLE');
%     caxis([zmin,zmax]);
%     
%     subplot(3,length(exp_data.ts),tc+2*length(exp_data.ts));
%     pcolor(padarray(good_comp_xray_ts(:,:,tc),[1 1],'post'));
%     title('MLE-Exp');
%     caxis([zmin,zmax]);
%     
%   end
  ...
  for tc = 1:length(exp_data.ts)
    subplot(3,length(exp_data.ts),tc);
    pcolor(padarray(exp_data.exp_xray_ts(:,:,tc),[1 1],'post'));
    title('Exp');
    caxis([zmin,zmax]);
    ...
    subplot(3,length(exp_data.ts),tc+2*length(exp_data.ts));
    pcolor(padarray(abs(mle_comp_xray_ts(:,:,tc)-exp_data.exp_xray_ts(:,:,tc)),...
      [1 1],'post'));
    title('MLE-Exp');
    caxis([zmin,zmax]);
